...
If a TOML file references a version (e.g., `version.ref = "kotlin"`) that is meant to be injected programmatically via `version("kotlin", "2.4.10-RC2")` in settings, the version must **not** appear in a `[versions]` block.
